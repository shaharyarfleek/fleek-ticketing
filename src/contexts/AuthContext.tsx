...
// Initial mock users - these will be merged with stored users
...
const getUsersFromStorage = (): AuthUser[] => {
  try {
    ...
    if (storedUsers) {
      ...
      const existingUsernames = parsed.map((u: AuthUser) => u.username.toLowerCase());
      const existingEmails = parsed.map((u: AuthUser) => u.email.toLowerCase());
      
      const newInitialUsers = initialMockUsers.filter(user => 
        !existingUsernames.includes(user.username.toLowerCase()) &&
        !existingEmails.includes(user.email.toLowerCase())
      );
      
      return [...parsed, ...newInitialUsers];
    }
  } catch (error) {
