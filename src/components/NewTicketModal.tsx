...
import { departments, users, getAutoAssignedUser } from '../data/mockData';
...
import { OrderData } from '../services/bigquery';
...
export const NewTicketModal: React.FC<NewTicketModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  // Fetch orders from BigQuery when modal opens
  useEffect(() => {
    if (isOpen) {
      fetchOrders();
    }
  ...
  const fetchOrders = async () => {
    ...
    setOrdersError(null);
    try {
      const orderData = await OrderApiService.getOrderLineIds();
      setOrders(orderData);
    } catch (error) {
      console.error('Failed to fetch orders:', error);
      setOrdersError('Failed to load orders. Using fallback data.');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Auto-assign based on issue category or manual selection
    let selectedAssignee: User | undefined;
    if (assigneeId) {
      selectedAssignee = users.find(u => u.id === assigneeId);
    } else {
      // Auto-assign based on issue category
      const autoAssigned = getAutoAssignedUser(departmentId, issueCategory);
      selectedAssignee = autoAssigned || undefined;
    }
  ...
  const handleOrderNumberSelect = async (orderNum: string) => {
    ...
    if (selectedOrder) {
      ...
      // If not found in cached data, try to fetch from BigQuery
      try {
        const orderDetails = await OrderApiService.getOrderDetails(orderNum);
        if (orderDetails) {
  ...
  const filteredUsers = users.filter(user => 
    (user.name && user.name.toLowerCase().includes(assigneeSearch.toLowerCase())) ||
    (user.email && user.email.toLowerCase().includes(assigneeSearch.toLowerCase())) ||
    (user.department && user.department.name && user.department.name.toLowerCase().includes(assigneeSearch.toLowerCase()))
  );

  // Filter order numbers based on search
  const filteredOrders = orders.filter(order =>
    order.orderLineId && order.orderLineId.toLowerCase().includes(orderNumberSearch.toLowerCase())
  );
          ...
          <div className={`p-6 rounded-xl border-2 transition-all duration-200 ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <div className="relative" ref={orderDropdownRef}>
                  ...
                  {showOrderNumberDropdown && (
                    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                      ...
                      {ordersError && (
                        <div className="px-4 py-2 text-amber-600 text-center border-b border-gray-100">
                          <div className="text-xs">{ordersError}</div>
                        </div>
                      )}
                      ...
                      {!ordersLoading && filteredOrders.map((order) => (
                        <button
                          ...
                          type="button"
                          onClick={() => handleOrderNumberSelect(order.orderLineId)}
                          className={`w-full px-4 py-2 text-left hover:bg-gray-50 flex items-center justify-between transition-colors duration-200 ${
                            order.orderLineId === orderNumber ? 'bg-blue-50 text-blue-700' : ''
                          }`}
                        >
                          <div className="flex items-center">
                            <Package className="w-4 h-4 mr-2 text-gray-400" />
                            <span>{order.orderLineId}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <span className="text-sm text-gray-500">
                              {order.currency || 'GBP'} {order.orderValue?.toFixed(2)}
                            </span>
                            {order.orderLineId === orderNumber && (
                ...
                <p className="text-xs text-gray-500 mt-1">🔍 Searchable field • 📊 Data from BigQuery API</p>
                {!ordersError && orders.length > 0 && (
                  <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded-lg">
                    <div className="text-xs text-green-700">
                      <strong>Connected to BigQuery:</strong> Real-time order data via backend API
                    </div>
                  </div>
                )}
                {ordersError && (
                  <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded-lg">
                    <div className="text-xs text-amber-700">
                      <strong>API Connection:</strong> Using fallback data (ensure backend is running on port 3001)
                    </div>
                  </div>
                )}
              </div>
