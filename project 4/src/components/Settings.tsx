...
export const Settings: React.FC<SettingsProps> = () => {
  ...
  const renderUserManagement = () => {
    // Always get users from localStorage directly - no role restrictions for now
    let allUsers: any[] = [];
    try {
      const storedUsers = localStorage.getItem('fleek_users_database');
      if (storedUsers) {
              ...
              <div className="space-y-3">
                ...
                <div className="p-3 bg-green-50 border border-green-200 rounded-lg text-left">
                  <p className="text-sm text-green-700">
                    <strong>Users in localStorage:</strong> {allUsers.length} users found
                  </p>
                </div>
              </div>
            </div>
    ...
    const handleBlockUser = async (user: AuthUser) => {
      ...
      setIsLoading(true);
      try {
        if (authState.blockUser) {
          await authState.blockUser(user.id, blockReason);
        }
    ...
    const handleUnblockUser = async (user: AuthUser) => {
      setIsLoading(true);
      try {
        if (authState.unblockUser) {
          await authState.unblockUser(user.id);
        }
      } catch (error) {
    ...
    const handleDeleteUser = async (user: AuthUser) => {
      ...
      setIsLoading(true);
      try {
        if (authState.deleteUser) {
          await authState.deleteUser(user.id);
        }
      } catch (error) {
          ...
          <div className="space-y-4">
            {usersLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-slate-500 mb-4">No registered users found in localStorage.</p>
                <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <p className="text-sm text-yellow-700">
                    <strong>Troubleshooting:</strong><br/>
                    1. Make sure users have signed up through the registration form<br/>
                    2. Check browser console for any errors<br/>
                    3. Try refreshing the page<br/>
                    4. Check if localStorage contains 'fleek_users_database' key
                  </p>
                </div>
              </div>
            ) : (
